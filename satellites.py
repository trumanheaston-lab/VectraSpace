...
async def run_scan(
    ...
    async def event_stream():
        ...
        try:
            ...
            yield _ev({"type": "progress", "pct": 15, "text": "Starting orbital scan..."})
            await asyncio.sleep(0)
            ...
            for c in result["conjunctions"]:
                ...
                conj_json.append({
                    ...
                    "debris": c.debris, "maneuver": maneuver_data, "midpoint": mid,
                })
...
def download_cdm(idx: int, request: Request):
    ...
    c = Conjunction(sat1=r["sat1"],sat2=r["sat2"],regime1=r["regime1"],regime2=r["regime2"],
                    min_dist_km=r["min_dist_km"],time_min=r["time_min"],pc_estimate=r["pc_estimate"])
...
def download_all_cdms(request: Request):
    ...
    with zipfile.ZipFile(buf, "w", zipfile.ZIP_DEFLATED) as zf:
        for i, row in enumerate(rows):
            ...
            s1 = str(r["sat1"] or "UNK")[:8].replace(" ","_")
            s2 = str(r["sat2"] or "UNK")[:8].replace(" ","_")
            zf.writestr(f"CDM_{i+1:03d}_{s1}_{s2}.cdm", generate_cdm(c, r["run_time"]))
    buf.seek(0)
...
async def satellite_info(sat_name: str):
    ...
    if not api_key:
        ...
        return JSONResponse({"fullName": clean, "noradId": None, "country": "Unknown",
...
async def simulate_debris(
    ...
    try:
        ...
        last = None
    if not last:
        return JSONResponse({"error": "No scan results — run a scan first."}, status_code=400)
    ...
    if not parent:
        return JSONResponse({"error": f"'{sat_name}' not found in last scan."}, status_code=404)
    ...
    conj_json   = [{"sat1":c.sat1,"sat2":c.sat2,"regime1":c.regime1,"regime2":c.regime2,
                    "min_dist_km":c.min_dist_km,"time_min":c.time_min,"pc_estimate":c.pc_estimate,
                    "covariance_source":c.covariance_source,"debris":True,"maneuver":None,
                    "midpoint":[0,0,400000]} for c in debris_conj if c.debris]
...
async def submit_feedback(request: Request):
    ...
    entry = {
        ...
        "type":      fb_type, "message": message, "email": email, "user": user,
    }
